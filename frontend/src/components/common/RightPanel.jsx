...
const RightPanel = () => {
  ...
  const [displayedAffiliation, setDisplayedAffiliation] = useState("");
  const [timer, setTimer] = useState(0);

  useEffect(() => {
    const savedTime = localStorage.getItem("timer");
    if (savedTime) {
      setTimer(Number(savedTime)); // Restore the last saved timer
    }
  }, []);

  useEffect(() => {
    if (currentUser) {
      setDisplayedAffiliation(currentUser.politicalAffiliation || "Other");
      const now = Date.now();
      const savedTime = localStorage.getItem("timer");
        ...
        <div className='flex flex-col gap-4'>
          ...
          {!isSuggestedUsersLoading &&
            suggestedUsers?.map((user) => {
              // Define the border color based on political affiliation
              const borderColor =
                user.politicalAffiliation === "conservative"
                  ? "border-red-600" // Standard red
                  : user.politicalAffiliation === "liberal"
                  ? "border-blue-600" // Standard blue
                  : "border-gray-600"; // Standard gray
